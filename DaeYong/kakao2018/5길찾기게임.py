...
def solution(nodeinfo):
    ...
    def postord_circ(node):
        nonlocal postorder_circulation
        if node.left is not None:
            postord_circ(node.left)
        if node.right is not None:
            postord_circ(node.right)
        postorder_circulation.append(node.n)

    postord_circ(tree.root)
